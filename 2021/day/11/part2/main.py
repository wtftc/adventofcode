    ...
    def step(self, i):
        ...
        for x in range(LEN_X):
            for y in range(LEN_Y):
                ...
                if not octo.flashed:
                    allFlashed = False
                octo.finishIteration()

        if self.firstAllFlash == -1 and allFlashed:
            self.firstAllFlash = i
...
if __name__ == '__main__':
    lines = readFile('C:/Users/bill/Code/projects/adventofcode/2021/day/11/part2/example.txt')
    breakpoint()
    exampleMap = OctoMap.processInput(lines)
    exampleMap.iterate(1000)
    firstFlash = exampleMap.firstAllFlash
    assert firstFlash == 195

    lines = readFile('C:/Users/bill/Code/projects/adventofcode/2021/day/11/part2/input.txt')
    inputMap = OctoMap.processInput(lines)
    inputMap.iterate(1000)
    firstAllFlash = inputMap.firstAllFlash
    print(f'firstAllFlash: {firstAllFlash}')
